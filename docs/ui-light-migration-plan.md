# UI シンプル化プラン (ダーク紫基調維持)

> ⚠️ 方針変更 (2026-05-?): 当初「白基調」で進めたが、ユーザー方針で
> **ダーク紫基調を完全に維持** に修正。表示パターン (大きめカード +
> 余白広め + アイコン+名前+人数) のみシンプル化する方向に変更した。
> SIMPLE_COLORS は dark 値 (`pageBg: #0a0a18` など) に書き換え済み。
> 本ドキュメント内の「light 化」「白カード」「light gray」等の表現は
> 旧方針時点の記述。色は **すべてダーク紫基調** で運用する。
