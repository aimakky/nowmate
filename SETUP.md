...
git init
git add .
git commit -m "feat: nowmate MVP initial release"
```

GitHub.com でリポジトリを新規作成して、表示されるコマンドを実行：
```
git remote add origin https://github.com/あなたのID/nowmate.git
git branch -M main
...
🎉 数分でURLが発行されます（例: `https://nowmate-xxxx.vercel.app`）
