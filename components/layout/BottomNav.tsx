...
import { Layers, Bell, MessageSquare } from 'lucide-react'
...
const NAV_ITEMS = [
  { href: '/timeline',      label: 'TL',      icon: Layers,        live: false, activeColor: '#39FF88', custom: null },
  { href: '/guilds',        label: 'ギルド',   icon: null,          live: false, activeColor: '#27DFFF', custom: 'shield' },
  { href: '/guild',         label: 'ゲーム村', icon: null,          live: true,  activeColor: '#8B5CF6', custom: 'gamepad' },
  { href: '/chat',          label: 'チャット', icon: MessageSquare, live: false, activeColor: '#FF4FD8', custom: null },
  { href: '/notifications', label: '通知',    icon: Bell,          live: false, activeColor: '#FFC928', custom: null },
]
  ...
  return (
    ...
    >
      <div className="max-w-[430px] mx-auto flex items-center h-16">
        ...
        {NAV_ITEMS.map(({ href, label, icon: Icon, live, activeColor, custom }) => {
          const active = pathname === href || pathname.startsWith(href + '/')
          const badge  = badges[href] ?? 0

          const renderIcon = () => {
            if (custom === 'shield') return <GuildShieldIcon size={22} active={active} />
            if (custom === 'gamepad') return <GamepadIcon size={22} active={active} />
            if (Icon) return (
              <Icon
                size={20}
