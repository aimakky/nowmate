...
export default function InvitePage() {
  ...
  const [nowjpId, setNowjpId] = useState<string | null>(null)
  ...
  useEffect(() => {
    async function load() {
      ...
      if (data?.nowjp_id) setNowjpId(data.nowjp_id)
    }
    load()
  }, [])

  const baseUrl = typeof window !== 'undefined' ? window.location.origin : 'https://getnowjp.com'
  const shareUrl = nowjpId ? `${baseUrl}/signup?ref=${nowjpId}` : `${baseUrl}/signup`
  const tweetText = `Surviving Japan alone is hard 🗾 I use nowjp — step-by-step setup guides + connect with expats who've been there. Join free 👇`
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col max-w-[430px] mx-auto">
      ...
      {nowjpId && (
        <section className="px-5 pb-6">
          <p className="text-xs text-center text-gray-400">Your nowjp ID: <span className="font-mono font-bold text-gray-600">#{nowjpId}</span></p>
        </section>
      )}
