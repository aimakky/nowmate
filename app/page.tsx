...
const GENRES = [
  { id: 'FPS・TPS',       emoji: '🎯', color: '#ef4444', bg: '#fef2f2' },
  { id: 'RPG',            emoji: '⚔️', color: '#8b5cf6', bg: '#f5f3ff' },
  { id: 'アクション',     emoji: '🔥', color: '#f97316', bg: '#fff7ed' },
  { id: 'スポーツ',       emoji: '⚽', color: '#10b981', bg: '#ecfdf5' },
  { id: 'スマホゲーム',   emoji: '📱', color: '#ec4899', bg: '#fdf2f8' },
  { id: 'シミュレーション',emoji: '🧠', color: '#0891b2', bg: '#ecfeff' },
  { id: 'パズル・カジュアル',emoji:'🧩', color: '#f59e0b', bg: '#fffbeb' },
  { id: 'インディー',     emoji: '🌱', color: '#059669', bg: '#ecfdf5' },
  { id: 'レトロゲーム',   emoji: '🕹️', color: '#6366f1', bg: '#eef2ff' },
  { id: '雑談・その他',   emoji: '💬', color: '#64748b', bg: '#f8fafc' },
]
    ...
    <div className="min-h-screen flex flex-col max-w-[430px] mx-auto" style={{ background: '#0a0a0f' }}>
      ...
      <section className="px-5 pt-10 pb-10 text-center flex flex-col items-center relative overflow-hidden">
        ...
        <div className="relative inline-flex items-center gap-2 rounded-full px-4 py-1.5 mb-6"
          ...
          <span className="relative flex w-2 h-2">
            <span className="absolute inline-flex h-full w-full rounded-full opacity-75 animate-ping" style={{ background: '#a78bfa' }} />
        ...
        <Link href="/signup"
          className="relative w-full py-4 rounded-2xl font-extrabold text-base text-center active:scale-[0.98] transition-all mb-3 overflow-hidden"
      ...
      <section className="px-5 pb-10">
        ...
        <div className="grid grid-cols-5 gap-2">
          {GENRES.map(g => (
            <div key={g.id} className="flex flex-col items-center gap-1.5 rounded-2xl py-3 px-1"
              style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
              <span className="text-xl">{g.emoji}</span>
              <span className="text-[8px] font-bold text-center leading-tight" style={{ color: '#94a3b8' }}>
                {g.id.length > 6 ? g.id.slice(0, 6) + '…' : g.id}
              </span>
            </div>
          ))}
        </div>
      ...
      <section className="px-5 pb-10">
        <div className="rounded-3xl overflow-hidden"
          style={{ background: 'linear-gradient(135deg,#0f0f1a 0%,#1a0a2e 100%)', border: '1px solid rgba(139,92,246,0.2)' }}>
          <div className="px-5 pt-6 pb-4 text-center">
            <p className="text-[10px] font-bold uppercase tracking-widest mb-1" style={{ color: 'rgba(139,92,246,0.6)' }}>
              vs Discord
            </p>
          ...
          <div className="px-4 pb-6 space-y-3">
            {[
              ...
              { problem: '仲間募集が埋もれてどこにあるか不明', solution: '仲間募集カテゴリ専用フォーム', icon: '🤝' },
            ].map(item => (
        ...
        <div className="space-y-3">
          {[
            ...
            },
            {
              emoji: '📊',
              title: '村内投票',
              desc:  '「今夜どのゲームやる？」をギルド内で投票。リアルタイムで結果が棒グラフで見える。',
              color: '#06b6d4',
            ...
            },
            {
              emoji: '🏆',
              title: 'ジャンルマスター称号',
              desc:  '同ジャンルのギルドに3つ以上参加するとジャンルマスター称号が解除。プロフィールに表示される。',
              color: '#f59e0b',
            ...
            },
            {
              emoji: '🧊',
              title: 'ストリーク＆凍結',
              desc:  '毎日ログインでストリークが続く。7日連続で凍結アイテム獲得 — 1日空いても自動でストリークを守る。',
              color: '#10b981',
      ...
      <section className="px-5 pb-10">
        <div className="rounded-3xl overflow-hidden"
          style={{ background: 'linear-gradient(135deg,#111827 0%,#1f2937 100%)', border: '1px solid rgba(255,255,255,0.08)' }}>
          <div className="px-5 pt-6 pb-4">
            <p className="text-[10px] font-bold uppercase tracking-widest mb-1" style={{ color: '#6b7280' }}>Trust Tier</p>
              ...
              {[
                ...
                <div key={t.label} className="flex items-center gap-3 rounded-xl px-3 py-2.5"
                  ...
                  <div className="flex-1 min-w-0 flex items-center justify-between gap-2">
                    <span className="text-xs font-bold flex-shrink-0" style={{ color: t.color }}>{t.label}</span>
                    <span className="text-[10px] truncate" style={{ color: '#6b7280' }}>{t.desc}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section className="px-5 pb-10">
        ...
        <div className="space-y-3">
          {[
            ...
            {
              text: '仲間募集フォームがちゃんと機能ごとに分かれてるから、見つけやすい。他のSNSだと流れちゃう。',
      ...
      <section className="px-5 pb-10">
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={item.text} className="flex items-center gap-3 rounded-2xl px-4 py-3"
      ...
      <section className="px-5 pb-14">
        <div className="rounded-3xl p-8 text-center relative overflow-hidden"
          style={{ background: 'linear-gradient(135deg,#1e0a3c 0%,#2d1b69 50%,#1e3a5f 100%)',
      ...
      <section className="px-5 pb-6">
        <Link href="/for-business"
          ...
          <div className="flex-1">
            ...
            <p className="text-xs" style={{ color: '#64748b' }}>ゲーム企業・コミュニティ運営向けプラン →</p>
          </div>
      ...
      {/* ── フッター ── */}
      <footer className="border-t px-5 py-6 text-center" style={{ borderColor: 'rgba(255,255,255,0.08)', background: '#0a0a0f' }}>
        ...
        <p className="text-xs mb-3" style={{ color: '#475569' }}>20歳以上限定の大人ゲームコミュニティ</p>
