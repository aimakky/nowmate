  ...
  function NotifRow({ n, glowColor }: { n: Notif; glowColor?: string }) {
    ...
    return (
      <button
        ...
        style={unread ? {
          background: glowColor ? `${glowColor}10` : 'rgba(255,79,216,0.08)',
        } : {}}
      >
        ...
        {unread && (
          <span className="w-2 h-2 rounded-full flex-shrink-0 mt-1.5"
            style={{ background: glowColor ?? '#FF4FD8', boxShadow: `0 0 6px ${glowColor ?? '#FF4FD8'}` }} />
        )}
      </button>
    )
  }
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen" style={{ background: '#080812' }}>
      ...
      <div className="sticky top-0 z-10 px-4 pt-12 pb-3 backdrop-blur-md"
        style={{ background: 'rgba(8,8,18,0.92)', borderBottom: '1px solid rgba(255,79,216,0.2)' }}>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0"
              style={{ border: '2px solid rgba(255,79,216,0.6)', background: 'rgba(255,79,216,0.1)' }}>
              <User size={16} style={{ color: '#FF4FD8' }} />
            </div>
            <div>
              <p className="text-[10px] font-bold tracking-widest uppercase" style={{ color: '#FF4FD8' }}>NOTIFICATIONS</p>
          ...
          {unreadCount > 0 && (
            <span className="flex items-center gap-1.5 text-[10px] font-bold px-3 py-1.5 rounded-full"
              style={{ background: 'rgba(255,79,216,0.15)', color: '#FF4FD8', border: '1px solid rgba(255,79,216,0.35)' }}>
              <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: '#FF4FD8' }} />
              {unreadCount}件未読
            </span>
          )}
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center px-6" style={{ minHeight: 'calc(100vh - 120px)' }}>
          <div className="w-full max-w-xs rounded-3xl p-8 flex flex-col items-center text-center"
            style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,79,216,0.18)' }}>
            {/* Bell icon with sparkles */}
            <div className="relative mb-6">
              {/* Sparkle dots */}
              <span className="absolute -top-2 -right-2 text-[10px] font-bold" style={{ color: 'rgba(255,79,216,0.6)' }}>+</span>
              <span className="absolute -bottom-1 -left-3 text-[10px] font-bold" style={{ color: 'rgba(255,79,216,0.4)' }}>+</span>
              <span className="absolute top-1 -left-4 text-[8px] font-bold" style={{ color: 'rgba(255,79,216,0.5)' }}>+</span>
              <span className="absolute -top-3 left-3 text-[8px] font-bold" style={{ color: 'rgba(255,79,216,0.3)' }}>+</span>
              <div
                className="w-[72px] h-[72px] rounded-2xl flex items-center justify-center"
                style={{
                  background: 'linear-gradient(135deg, rgba(255,79,216,0.25) 0%, rgba(255,77,144,0.2) 100%)',
                  border: '1px solid rgba(255,79,216,0.4)',
                  boxShadow: '0 0 30px rgba(255,79,216,0.25)',
                }}
              >
        ...
        <div className="pb-28 pt-2">
          ...
          {reactionNotifs.length > 0 && (
            <div className="px-4 mb-4">
              <div className="flex items-center gap-2 py-3">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: 'rgba(255,79,216,0.9)' }}>
                  返信・反応
                </span>
              </div>
              <div className="rounded-2xl overflow-hidden"
                style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,79,216,0.2)', boxShadow: '0 4px 24px rgba(0,0,0,0.3)' }}>
                <div className="divide-y" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
                  {reactionNotifs.map(n => (
                    <NotifRow key={n.id} n={n} glowColor="#FF4FD8" />
                  ))}
                </div>
              </div>
            </div>
          ...
          {voiceNotifs.length > 0 && (
            <div className="px-4 mb-4">
              <div className="flex items-center gap-2 py-3">
                <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: 'rgba(255,79,216,0.8)' }}>🎙️ ゲーム村通話</span>
              </div>
              <div className="rounded-2xl overflow-hidden"
                style={{ background: 'rgba(255,79,216,0.06)', border: '1px solid rgba(255,79,216,0.25)', boxShadow: '0 4px 24px rgba(255,79,216,0.15)' }}>
                <div className="divide-y" style={{ borderColor: 'rgba(255,79,216,0.12)' }}>
                  {voiceNotifs.map(n => (
                    <NotifRow key={n.id} n={n} glowColor="#FF4FD8" />
                  ))}
                </div>
          ...
          {otherNotifs.length > 0 && (
            <div className="px-4 mb-4">
              ...
              <div className="rounded-2xl overflow-hidden"
                style={{ background: 'rgba(255,79,216,0.03)', border: '1px solid rgba(255,79,216,0.1)' }}>
                <div className="divide-y" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
                  {otherNotifs.map(n => (
