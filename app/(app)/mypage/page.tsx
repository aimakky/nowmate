...
import { Settings, LogOut, ChevronRight, Crown, Users, Copy, Check } from 'lucide-react'
import { VILLAGE_TYPE_STYLES } from '@/components/ui/VillageCard'
import { INDUSTRIES } from '@/lib/guild'

type ProfileTab = 'villages' | 'following' | 'followers'
...
export default function MyPage() {
  ...
  const [activeTab,      setActiveTab]      = useState<ProfileTab>('villages')
    ...
    <div className="max-w-md mx-auto min-h-screen bg-stone-50">
      ...
      <div className="relative">
        ...
        <div className="absolute left-4" style={{ bottom: -36 }}>
          <div
            className="w-20 h-20 rounded-full border-4 border-white overflow-hidden bg-stone-100 flex items-center justify-center shadow-md"
          >
            {profile.avatar_url
      ...
      <div className="pt-14 px-4 pb-3 bg-white border-b border-stone-100">
        ...
        <div className="flex gap-5 mt-3">
          ...
          <div className="flex items-center gap-1.5">
            <span className="font-extrabold text-stone-900 text-sm">{postCount}</span>
      ...
      <div className="flex bg-white border-b border-stone-100 sticky top-0 z-10">
        {([
          { id: 'villages',  label: '参加中の村', count: villageCount },
          { id: 'following', label: 'フォロー中', count: followingCount },
          { id: 'followers', label: 'フォロワー',  count: followersCount },
        ] as { id: ProfileTab; label: string; count: number }[]).map(tab => (
          <button
            key={tab.id}
            ...
            {followerUsers.length === 0 ? (
              ...
              return (
                <div key={u.id} className="flex items-center gap-3 px-4 py-3.5">
                  ...
                  {isFollowing ? (
                    <span className="flex-shrink-0 text-xs font-bold px-3 py-1.5 rounded-full bg-stone-100 text-stone-400">
                      相互
                    </span>
                  ) : (
                    <button
