...
export default function ProfilesClient({ initialRows, initialHasMore, initialError }: Props) {
  ...
  function loadPage(nextPage: number) {
    startTransition(async () => {
      const res = await fetchProfilesPage(nextPage)
      setRows(res.rows)
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-5">
        ...
        {error && (
          <div className="bg-red-950/40 border border-red-900/50 rounded-2xl p-3 text-xs text-red-400">
            {error === 'unauthorized' ? '管理者権限が必要です' : error}
          </div>
        )}
        ...
        <div className="flex items-center justify-between">
          <div className="text-[11px] text-white/40">
            ...
            表示 <span className="text-white font-bold">{filtered.length}</span> / 取得 <span className="text-white font-bold">{rows.length}</span>
          </div>
