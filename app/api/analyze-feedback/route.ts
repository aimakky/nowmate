...
export async function POST(req: Request) {
  ...
  const response = await client.messages.create({
    ...
    max_tokens: 1024,
    system: 'You are a product manager analyzing user feedback for "VILLIA" — a Japan survival kit app for foreign residents in Japan. Return only valid JSON, no markdown.',
    messages: [{
      role: 'user',
