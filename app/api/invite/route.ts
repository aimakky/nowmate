...
export async function POST(req: Request) {
  const { inviteeId, inviter休憩村Id } = await req.json()
  if (!inviteeId || !inviter休憩村Id) {
  ...
  await supabase.from('invites').insert({
    inviter_VILLIA_id: inviter休憩村Id,
  ...
  await Promise.all([
    supabase.from('profiles')
      .update({ premium_until: premiumUntil, invited_by: inviter休憩村Id })
      .eq('id', inviteeId),
    supabase.from('profiles')
      .update({
